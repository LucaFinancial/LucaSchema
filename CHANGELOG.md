...
- **Journal Entry Grouping**: Added `journalEntryId` field to link related debit/credit postings in double-entry bookkeeping
  - **Terminology Note**: In this library, `Transaction` objects represent individual "postings" (journal entry lines), and `journalEntryId` links multiple postings into complete "journal entries"
...
- **Journal Entry Utilities**:
  ...
  - `groupTransactionsByJournalEntry()` - Groups postings by their journal entry ID
  - `validateAllJournalEntries()` - Batch validation of multiple journal entries
  - Deprecated alias: `groupTransactionsByGroupId` (use `groupTransactionsByJournalEntry` instead)
...
- Example journal entries demonstrating linked debit/credit postings
- Comprehensive test suite for journal entry validation and account hierarchy (122 tests total)
- Full documentation of new features in README with accounting terminology clarifications

### Changed

- Transaction schema now includes optional `journalEntryId` field for linking postings into journal entries
- Schema enumeration updated to include 'account' schema type
- Test utilities enhanced with account creation helpers
- Documentation updated to clarify accounting terminology (postings vs. journal entries)
